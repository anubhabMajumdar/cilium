...
func createMap(lc cell.Lifecycle, registry *metrics.Registry, l *slog.Logger) *SubnetTopology {
...
func create(params Params) *SubnetTopology {
	st := createMap(params.Lifecycle, params.Registry, params.Logger)
...
func (st *SubnetTopology) AddCidr(cidr string, id int) error {
	...
	value := NewSubnetValue(uint32(id))
...
func (st *SubnetTopology) DeleteCidr(cidr string) error {
	_, ipnet, err := net.ParseCIDR(cidr)
	if err != nil {
		return fmt.Errorf("failed to parse CIDR %s: %w", cidr, err)
	}
	...
	key := NewSubnetKey(ip, mask)
	if err := st.m.Delete(&key); err != nil {
		return fmt.Errorf("failed to delete subnet %s: %w", key.String(), err)
	}
	delete(st.Subnets, key)
	st.l.Info("Subnet deleted", slog.String("key", key.String()))
	return nil
}

func (st *SubnetTopology) LoadSubnets(subnets []subnet) error {
	// Delete the existing subnets before loading new ones.
	for k := range st.Subnets {
		if err := st.m.Delete(&k); err != nil {
			st.l.Error("Failed to delete existing subnet", slog.String("cidr", k.String()), slog.String("error", err.Error()))
		}
	}
	st.l.Info("Cleared existing subnets from topology")
	st.Subnets = make(map[SubnetKey]SubnetValue)
	// Load the new subnets into the topology.
	for _, subnet := range subnets {
		st.AddCidr(subnet.cidr, subnet.id)
	}
	st.l.Info("Subnets loaded", slog.Int("count", len(subnets)))
	return nil
}
