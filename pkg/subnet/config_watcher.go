...
func NewConfigWatcher(params Params) error {
	...
	params.Lifecycle.Append(cell.Hook{
		OnStart: func(_ cell.HookContext) error {
			if err := cw.cm.m.OpenOrCreate(); err != nil {
				cw.logger.Error("Failed to create BPF map", logfields.Error, err)
				return err
			}
			cw.logger.Info("BPF map created", "name", cw.cm.m.Name())
...
func (cw *configWatcher) reload() {
	...
	cw.logger.Info("Syncing new subnet topology to BPF map")
	if err := cw.cm.LoadSubnets(t.subnets); err != nil {
		cw.logger.Error("failed to load subnets into BPF map", logfields.Error, err)
		return
	}
	// for _, subnet := range t.subnets {
	// 	if err := cw.cm.AddCidr(subnet.cidr, subnet.id); err != nil {
	// 		cw.logger.Error("failed to add CIDR to subnet topology", logfields.Error, err, "cidr", subnet.cidr)
	// 		continue
	// 	}
	// 	cw.logger.Info("CIDR added to subnet topology", "cidr", subnet.cidr, "id", subnet.id)
	// }
